...
const i18Obj = {
  ...
  bel: {
    ...
    "hero-text":
      "Захавайце шчырыя эмоцыі, рамантычныя перажыванні і шчаслівыя моманты жыцця разам з прафесійным фатографам",
    hire: "Запрасіць",
    "skill-title-1": "Фотаздымак",
    "skill-text-1": "Высакаякасныя фота ў студыі і на прыродзе",
    "skill-title-2": "Відэаздымка",
    "skill-text-2":
      "Захавайце лепшыя моманты, каб яны назаўсёды заставаліся з вамі",
    "skill-title-3": "Рэтуш",
...
const settingsAll = document.querySelector(".settings-all");
...
function addDisplay() {
  settingsAll.classList.toggle("active-setting");
  if (settingsAll.classList.contains("active-setting")) {
    settingsAll.style.display = "block";
  } else {
    settingsAll.style.display = "none";
  }
}
function addClassMute() {
...
volumeSlider.addEventListener("input", volumeColorChange);
volumeIcon.addEventListener("click", addClassMute);
